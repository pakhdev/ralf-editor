...
// Этот декоратор используются для инжекции зависимостей в классы. Допускается инжекция классов. Все статические
// методы инжектируемых классов окажутся доступными в декорируемом классе. Все обычные методы инжектируемых классов
// так же будут доступны в декорируемом классе. Из них будут созданы инстансы и созданы методы фасады в декорируемом
// классе. Контекст у этих инжектируемых методов будет собственный и так же у них будет запущен метод OnInit, если
// он присутствует. В инжектируемых классах так же можно использовать инжектируемые классы. В аргументах декоратора
// следует передать флаг forRoot: если декорируемый класс является основным классом Ralf, то это значение должно
// быть true, в противном случае false, но в этом случае первым аргументом конструктора декорируемого класса должнен
// быть экземпляр класс Ralf. Декорируемый класс сохраняет все свои свойства и методы, включая статические. Для
// подключения внешних классов существуют параметры actions и handlers. Их нужно передать в виде массива классов.
// Разница между ними в том, что в actions допускаются статические методы, а в handlers - нет. А в handlers
// допускается метод onInit, который будет запущен при автоматической инициализации экземпляра класса.
